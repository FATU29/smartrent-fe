// Script to convert existing province data to use shared utilities
// Run this once to clean up the data structure

import {
  normalizeVietnameseText,
  createSearchableDivision,
  Province,
} from './shared'
...
const rawProvinces = [
  ...
  { code: '66', name: 'Đăk Lăk' },
  { code: '67', name: 'Đăk Nông' },
...
export const VIETNAM_PROVINCES: Province[] = rawProvinces.map(
  createSearchableDivision,
)

// Utility functions
export const findProvinceByCode = (code: string): Province | undefined => {
...
export const findProvinceByName = (name: string): Province | undefined => {
  ...
  return VIETNAM_PROVINCES.find(
    (province) =>
      province.normalized.includes(normalized) ||
      normalized.includes(province.normalized),
  )
}
