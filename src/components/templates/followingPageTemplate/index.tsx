...
const FollowingPageTemplate: React.FC = () => {
  ...
  // The feed is always scoped to a specific followed person (the combined
  // "latest posts from everyone" view was removed), so default to the first
  // followed person once the list loads.
  const firstPersonId = filterPeople[0]?.userId
  React.useEffect(() => {
    if (!feedUserId && firstPersonId) {
      setFeedUserId(firstPersonId)
    }
  }, [feedUserId, firstPersonId])
              ...
              <div className='flex items-center gap-2 text-sm'>
                ...
                >
                  <SelectTrigger className='h-9 w-full sm:w-64'>
                    <SelectValue placeholder={t('filters.sidebarTitle')} />
                  </SelectTrigger>
                  <SelectContent>
